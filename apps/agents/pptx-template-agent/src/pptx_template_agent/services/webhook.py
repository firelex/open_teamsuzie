...
def get_agent_api_key(headers: Mapping[str, str]) -> str | None:
    """Extract a caller-provided agent API key from headers, if present."""
...
async def fire_completion_webhook(
    ...
    """POST a completion notification to the admin service.

    Resolves the webhook URL via admin's resolve-by-key endpoint. Matches the
    pptx-agent contract so the same admin can serve both agents."""
