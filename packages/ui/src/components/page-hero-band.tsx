...
export interface PageHeroBandStatusPill {
  tone: "live" | "read" | "beta"
  label: string
}

/**
 * Icon prop shape used by hero components — intentionally a structural type
 ...
 */
export type PageHeroBandIcon = React.ComponentType<{ className?: string }>

export interface PageHeroBandProps {
  icon: PageHeroBandIcon
  ...
  statusPill?: PageHeroBandStatusPill
   ...
   * rotated, bottom-left tilted) at low opacity to give the band its
   * layered texture. Defaults to `/assets/graph-effect.png`; pass `null`
   * to disable the watermark for a flat hero.
   */
...
const PILL_TONES: Record<PageHeroBandStatusPill["tone"], string> = {
...
const PILL_DOT: Record<PageHeroBandStatusPill["tone"], string> = {
  live: "bg-emerald-500",
  read: "bg-amber-500",
  beta: "bg-ev-500",
}
...
/**
 * Editorial hero band. The cyan brand gradient + watermark + kicker pill +
 * bold title combo that gives every Suzie department page its "this is one
 * product family" feel.
 *
 * Standalone and composable: use directly inside an `AppShellContent` for
 * apps that want the hero look without `PageShell`'s full-height scroll
 * wrapper. `PageShell` itself renders this internally.
 */
export function PageHeroBand({
  ...
  return (
    <section
      data-slot="page-hero-band"
      className={cn(
        "relative shrink-0 overflow-hidden border-b border-[var(--color-header-edge)]/60 bg-header-gradient",
        className,
      )}
    >
      {watermarkSrc && (
