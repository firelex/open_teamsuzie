...
| `compare_documents` / diff | `tools/diff.ts` + `diff-engine.ts` | ✓ ported, then split into two upstream tools sharing the same `runDocumentDiff` helper: `blackline_documents` (tracked-change `.docx`, rendered as continuous `<RedlinePanelContent>` in the side panel) and `compare_documents` (analytical events + stats, rendered as a two-column `<CompareTable>` in the side panel). The model picks one or both depending on whether the user wants the file, the analysis, or both. |
